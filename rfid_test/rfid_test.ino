...
char tag[] ="0600E2264B89"; // Replace with your own Tag ID
char tag1[] ="0600E2264B89"; // Replace with your own Tag ID
  ...
  if(Serial.available())// Check if there is incoming data in the RFID Reader Serial Buffer.
  {
      ...
      if(count == 12) // 
    {
      ...
      while(count<12 && flag !=0)  
      {
        ...
        flag = 2; // everytime the values match, we set the flag variable to 1
        else
        flag= 0; 
                               /* if the ID values don't match, set flag variable to 0 and 
                                  stop comparing by exiting the while loop */
    ...
    }else if(flag == 2) // If flag variable is 1, then it means the tags match
    {
      Serial.println("Access Allowed!");
      digitalWrite(RELAYPIN,HIGH);
      delay (1000);
